...
/**
 * Ktor 기반 단일 HTTP 클라이언트.
 *
 * 백엔드는 요청·응답 모두 camelCase 키를 사용하므로 [Json] 설정에서 추가 전략을 두지 않는다.
 * `/api/auth/mobile/exchange` 처럼 snake_case 키가 필요한 곳은 모델 측 `@SerialName` 으로 명시한다.
 *
 * 401 응답 시 백엔드가 refresh를 제공하지 않으므로 즉시 [AuthState.handleUnauthorized]로 정리.
 */
object ApiClient {
    val json: Json = Json {
    ...
    /**
     * 모든 verb를 지원하는 일반화된 진입점. inline + reified 로 응답 타입을 자동 디코딩한다.
     */
    ...
    private fun HttpRequestBuilder.markUnused(): Unit = Unit
}
