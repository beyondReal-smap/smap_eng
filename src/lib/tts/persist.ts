...
// 하나의 Promise로 병합한다(Kokoro가 같은 문장을 두 번 처리하지 않도록).
import { mkdir, stat, writeFile } from 'node:fs/promises';
import path from 'node:path';
import { eq } from 'drizzle-orm';
import { db, schema } from '@/lib/db';
import { updatePassageAudio } from '@/lib/db/queries';
import { synthesize, TtsError } from './kokoro';
...
// /public/audio/passage-<id>.wav — Next.js 정적 서빙 그대로 재생.
...
// Kokoro는 한 번만 때리고, 호출자들은 동일한 Promise를 await한다.
...
export interface SynthesizeOptions {
  /**
   * true면 디스크 캐시를 우회하여 Kokoro를 다시 호출하고 wav를 덮어쓴다.
   * 사용처: reader의 다시듣기/자동복구 — 깨진 wav가 캐시되어 있을 때
   * 사용자가 "다시 만들어 달라"고 명시한 경로. 멱등 캐시(force=false)와는
   * 분리해 비싼 Kokoro 호출이 자동 재시도로 폭주하지 않도록 한다.
   */
  force?: boolean;
}
...
export function synthesizePassage(
  ...
  const promise = (async (): Promise<PersistedAudio> => {
    ...
    // DB만 보정한다. — Kokoro 불필요 호출 방지(MVP에서 가장 비싼 경로).
